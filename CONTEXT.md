...
**Faixa de Horário**:
Unidade de configuração da agenda pela qual a capacidade é consultada, identificada por `idTurno`. Um local tem várias faixas (ex. MANHÃ, TARDE).
_Evitar_: turno, período, janela.

**Sala**:
Espaço reservável dentro de uma faixa que carrega a capacidade nominal (`lotacaoMax` base) e a lista de postos de trabalho. Alvo do endpoint `capacidade-salas`. Formato do nome: `<SALA>_<TURNO>`.
_Evitar_: grupo, recurso.

**Posto de trabalho**:
Estação individual dentro de uma sala (`idPosto`). Pode ter teto próprio (`lotacaoMax ≠ 0`) ou não (`= 0` → capacidade resolvida no nível da sala).

**Override de capacidade**:
Exceção datada (`ajustesCapacidade`) que substitui o `lotacaoMax` base da sala durante um intervalo de datas, quando `ativo = 1`.
_Evitar_: alteração, ajuste, exceção.

**Capacidade do dia**:
A capacidade vigente resolvida para uma data específica: o override que cobre a data (se ativo), senão o `lotacaoMax` base da sala.

**Status da Reserva**:
Dimensão do ciclo de vida da reserva (`idSituacao`). IDs 1–5, 8 e 9 observados; 6 e 7 não confirmados.
...
Asserção, na ingestão, de que a contagem por status calculada de `reservas[]` bate com `extendedProps.totaisPorStatus` do bloco. Divergência sinaliza truncamento silencioso do array. O `totaisPorStatus` é chaveado por **bucket de workflow** (`reservado`, `cancelado`, `checkIn`, `checkOut`, `noShow`, `pendencia`), não por `idSituacao` — a comparação traduz id→bucket pelo mapa provisório `STATUS_ID_TO_BUCKET` (`ocupa_salas.canary`), no qual `9` ("Removido") cai em `cancelado`.
_Evitar_: totaisPorStatus por id, rótulo de status.
...
Linha silver com o teto no nível do posto para uma data, quando `posto.lotacaoMax ≠ 0`. Mantida separada de `capacidade_sala_dia` — a regra de combinação entre as duas ainda não foi observada.
