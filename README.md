...
ocorrência. Boa parte dos campos carrega sufixo `Erp`: o cadastro nem nasce aqui, é
replicado de um ERP a montante. Nada disso pode tocar o disco.
...
flowchart LR
    subgraph Fonte["ReservaSpace (fictícia — mock_server/)"]
        ...
        A2["POST /config/capacidade-salas<br/>capacidade por faixa"]
    end
...
(`GET /` → `/selecionar-contexto` → `/aplicar-contexto` → `GET /` → `/config/unidade`
→ `POST /config/capacidade-salas`), aplica o strip de PII, calcula o `hash_linha` e
