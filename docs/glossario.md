...
| **Faixa de Horário** | Unidade de configuração da agenda pela qual a capacidade é consultada (`idTurno`). Um local tem várias (ex.: MANHÃ, TARDE). |
...
| **Capacidade nominal** | `lotacaoMax` base da sala. |
...
| **Canário de integridade** | Asserção de que a contagem por status calculada bate com `extendedProps.totaisPorStatus` do bloco. Divergência = possível truncamento silencioso do array. |
