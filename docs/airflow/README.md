...
def ocupa_salas_dag():
    ...
    def collect(data_interval_start=None) -> dict:
        ...
        write_jsonl(run.linhas_novas, f"/data/ocupa_salas/{particao}/reservas.jsonl", append=True)
        write_jsonl(run.capacidade, f"/data/ocupa_salas/{particao}/capacidade.jsonl")
        state.save()  # só depois de gravar: se a escrita falhar, o próximo ciclo reemite
