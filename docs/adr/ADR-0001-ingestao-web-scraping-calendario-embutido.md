...
- **Nota de portfólio:** domínio e fonte são fictícios. O padrão "fonte = API REST
  com token" citado abaixo é um padrão de referência genérico (um framework de
  ingestão orientado a catálogo); o ADR registra por que esta fonte é uma
  **exceção** a ele.
...
- Os dados granulares de reserva vêm **embutidos como JSON dentro de um `<script>`
  ...
  `FullCalendar`.
- A capacidade nominal vem de `POST /config/capacidade-salas`, um por faixa de
...
2. **"Paginação"** é fatiar um array JSON de dentro de HTML — não é `page_number`
   nem `cursor`.
3. **Duas chamadas encadeadas** (`GET local` → lista de faixas → `POST capacidade-salas`
   por faixa).
...
- **Tabela de bloco:** metadados da faixa + `totaisPorStatus`, usada como canário de
  integridade na ingestão.
- **Tabela de capacidade:** resposta de `capacidade-salas` por faixa; grão = sala;
  `id_origem` composto `(idSala, idTurno)`; `ajustesCapacidade[]` e `postos[]` como
  JSON aninhado. Mesmo versionamento por `hash_linha`.

**O que muta num `idReserva` recoletado:** não é só o status —
`checkIn`/`checkOut` e seus `*Por`, `cancelado`/`canceladoPor`, `concluidoPor`,
`idPendencia`, e em remarcação `dataReserva`/`idSala`. `atualizadoEm` é
...
### D7 — Seleção de contexto de sessão (`/selecionar-contexto`)

**Achado do 1º bootstrap:** com conta de serviço (acesso a vários locais), o `GET /`
autenticado **redireciona para `/selecionar-contexto`**. Sem um local fixado na
...
1. `GET /` — se o corpo traz `id="seletorUnidade"`:
2. `GET /selecionar-contexto` → raspa o par CSRF das `<meta>`;
3. `POST /aplicar-contexto`, corpo `filtro={"escopo":"local","id":<id_local>}`,
...
`empresa` é opcional — vira um `POST /aplicar-contexto` extra com `escopo=empresa`
...
| Fixar local por query param em vez de `POST /aplicar-contexto` (D7) | Todas as rotas redirecionam até o contexto entrar na sessão pelo `POST` |
