...
| `idSituacao` | int | FK dimensão de status |
| `idPendencia` | int/null | quando "Concluído com Pendência" |
| `idSala` / `idPosto` / `idTurno` | str | FKs de capacidade (`idTurno` pode ser null) |
| `idAlocacao` | int | agrupador (uso não confirmado) |
...
| `canceladoPor` / `concluidoPor` | str/null | id do usuário |
| `criadoPor` | str | id do usuário que reservou |
...
|---|---|---|
| `idSala` | str | PK (com `idTurno`) |
| `idTurno` | str | |
| `lotacaoMax` | str→int | capacidade base da sala |
| `nome` | str | `<SALA>_<TURNO>` (rótulo — não é PII; renomeado para `nomeSala` nas fixtures) |
| `ajustesCapacidade[]` | json | overrides datados: `dataInicio`, `dataFim`, `lotacaoAjustada`, `ativo` |
| `postos[]` | json | `idPosto`, `lotacaoMax` (0 = sem teto de posto), `peso` |
