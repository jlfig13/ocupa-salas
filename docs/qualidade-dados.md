...
|---|---|---|---|
| **DQ-01** | `GET /` traz a tela autenticada (marcadores `csrf-token` + `agendaReservas`) | coletor (`_require_authenticated_page`) | `SessionExpiredError` — falha alta, pede re-bootstrap |
...
| **DQ-04** | Canário: contagem por bucket calculada = `extendedProps.totaisPorStatus` | `ocupa_salas.canary` | **marca** o bloco (`canario_falhou`), alerta, **não bloqueia** |
...
| **DQ-08** | `capacidade_do_dia > 0` para toda linha do fato com reservas | silver | linha sem capacidade → quarentena + alerta (sala sem `lotacaoMax` nem override) |
