...
flowchart LR
    subgraph Fonte["ReservaSpace (fictícia)"]
        A[Tela de calendário<br/>+ tela de configuração]
    end
    subgraph Orquestracao["Airflow (documentado em docs/airflow)"]
        B[Coletor ocupa_salas<br/>scraping do array events<br/>+ POST capacidade-salas]
...
| Componente | Papel |
|---|---|
| **Coletor** (`ocupa_salas`) | Extrai `events` (HTML) e capacidade (`POST capacidade-salas`). Aplica strip de PII, canário de integridade e versionamento por hash. Orquestração Airflow; credenciais em Connection / `OCUPA_SALAS_*`. |
