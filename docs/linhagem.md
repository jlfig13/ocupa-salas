...
```
ReservaSpace (HTML + capacidade-salas)
...
|---|---|---|
| **Vagas Disponíveis** | `stg_capacidade.lotacaoMax` + `ajustesCapacidade[]` | RN-03 → `capacidade_do_dia`; Σ no período |
| **Reservas Realizadas** | `stg_reservas.payload.idSituacao` ∈ {3,4} | RN-01/RN-02 → `qtd_ocupado`; Σ |
| **Reservado (futuro)** | `idSituacao = 1` | `qtd_consome_vaga_sinalizado`; Σ |
| **Cancelamentos + Faltas** | `idSituacao` ∈ {2,5,8,9} | `qtd_fora`; Σ |
...
| **`tem_override`** | `ajustesCapacidade[]` cobrindo a data com `ativo=1` | RN-03 |
| **`tem_status_provisional`** | `dim_status_kpi_role.provisional` (status 8, 9) | RN-10 |
| **`canario_falhou`** | `stg_reservas_bloco` marcado na ingestão | canário (contagem × `totaisPorStatus`) |
