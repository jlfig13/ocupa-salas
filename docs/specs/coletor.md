...
0. **Seleção de contexto (condicional — D7).** Se o `GET /` cai em
   `/selecionar-contexto` (marcador `id="seletorUnidade"`): `GET /selecionar-contexto` p/
   CSRF → `POST /aplicar-contexto` (`filtro={"escopo":"local","id":<id_local>}`,
   CSRF só no header) → repete `GET /`. `id_empresa` → POST extra.
1. `GET /` → fatiar `events` → reservas + metadados de bloco. Linhas
   `extendedProps.marcadorDia: true` ignoradas.
2. `GET /config/unidade?idLocal=<id>` → lista de `idTurno` do global
   `locais` → para cada faixa, `POST /config/capacidade-salas`.
...
`stg_reservas_bloco` (metadados + `totaisPorStatus`), `stg_capacidade` (resposta de
`capacidade-salas`; grão = sala; `id_origem` composto). Colunas de rastreio padrão.
...
× `totaisPorStatus`; divergência gera alerta sem bloquear (linha marcada).
...
`totaisPorStatus` por bloco; linhas `marcadorDia` ignoradas; `hash_linha` estável
