...
> engenharia reversa que um projeto desse tipo produz.
...
```js
jQuery('#agendaReservas').fullCalendar({
  ...
  "events": [ ... ]
})
```
...
metadado do bloco e, dentro de `extendedProps.reservas`, a lista de reservas
individuais daquela faixa.

Campos relevantes do bloco:

- `id` — pode ser `null` (primeiro item de cada dia, placeholder/cabeçalho —
  **ignorar no parsing**, identificável por `extendedProps.marcadorDia: true`)
- `title` — string do horário, ex. `"08:00 - 12:00"`
- `start`, `end` — timestamps do bloco
- `extendedProps.reservas` — array de reservas (ver 2.2)
- `extendedProps.totaisPorStatus` — contagem pré-agregada **por bucket de workflow**
  (`reservado`/`cancelado`/`checkIn`/`checkOut`/`noShow`/`pendencia`). Usado como
  canário de integridade, não como fonte primária de KPI.

### 2.2 Nível "reserva" (item de `extendedProps.reservas`)
...
| `idAlocacao` | agrupador (uso ainda não confirmado) |
| `idMembro` | FK para membro — **extrair só o id**, ver 2.3 |
| `idSituacao` | FK para status (dimensão mapeada, seção 3) |
| `idPendencia` | FK para pendência, quando status = "Concluído com Pendência" |
| `idPosto` | FK posto de trabalho |
| `idSala` | FK sala |
| `idTurno` | FK faixa de horário (pode vir `null` no payload do calendário) |
| `dataReserva` | timestamp com offset da reserva |
| `criadoPor` | id do usuário que reservou |
...
| `concluidoPor` | id do responsável pelo atendimento |

**Campos aninhados — ⚠️ tratamento especial (ver seção 4):**
`membro`, `sala`, `situacao`, `situacaoPendencia`.
...
## 3. Dimensão de status (`idSituacao`)

Extraída por varredura de uma amostra de reservas:

| id | nomeStatus | bucket `totaisPorStatus` (provisório) |
|---|---|---|
...
**Sobre `totaisPorStatus`:** vem como mapa `{bucket: n}` chaveado por **bucket de
workflow**, não por `idSituacao`. O canário (`ocupa_salas.canary`) traduz
...
**Origem dos dados:** vários campos do objeto `membro` trazem sufixo `Erp`
(`setorCodErp`, `centroCustoCodErp`, `codMembroErp`, `id_contrato_erp`, …) — indício
de que o cadastro não é nativo da plataforma de reserva, e sim **replicado de um ERP
...
## 5.1 Seleção de contexto — `/selecionar-contexto` + `/aplicar-contexto`

**Achado do 1º bootstrap contra produção.** Com uma conta de serviço (acesso a mais
de um local), o `GET /` autenticado **redireciona para `/selecionar-contexto`** — tela
"Selecione um Local / Selecione uma Empresa". Enquanto a sessão não tem um local
fixado, **nenhuma rota entrega o calendário**.

`/selecionar-contexto` é página normal: **200**, com o par CSRF nas `<meta>`. Cada
`<select>` (`#seletorUnidade`, `#seletorEmpresa`) dispara a mesma chamada:

```
POST /aplicar-contexto
Content-Type: application/x-www-form-urlencoded
X-Requested-With: XMLHttpRequest      ← obrigatório
X-CSRF-Token: <token>                  ← token do meta csrf-token de /selecionar-contexto

Body:
  filtro = {"escopo":"local","id":"1"}   (JSON stringificado; "empresa" para o filtro de empresa)
```

⚠️ Aqui o token CSRF vai **só no header** `X-CSRF-Token` — diferente de
`/config/capacidade-salas` (§6.1), onde vai também no corpo.
...
- **Passo condicional.** Conta com acesso a **um único local** cai direto no
  calendário. O coletor decide pelo corpo do `GET /`: se contém `id="seletorUnidade"`,
  roda a seleção e repete o `GET /`.
- **CSRF por sessão/deploy** — sempre raspar do `<meta>`, nunca fixar.

---

## 6. Endpoint de capacidade — `/config/capacidade-salas`
...
```
POST /config/capacidade-salas
Content-Type: application/x-www-form-urlencoded
X-Requested-With: XMLHttpRequest      ← obrigatório
X-CSRF-Token: <token>                  ← obrigatório

Body:
  filtro      = {"idTurno":"1071"}   (JSON stringificado)
...
`idTurno` vem do array `locais[].turnos[]`, embutido no carregamento de
`GET /config/unidade?idLocal=<id>` (variável JS global `locais`).
...
[
  {
    ...
    "idTurno": "1071",
    "ajustesCapacidade": [
      { "idAjuste": "4", "lotacaoAjustada": "16", "dataInicio": "2026-08-14", "dataFim": "2026-08-14", "ativo": "1" }
    ],
    "postos": [
      { "idPosto": "401P1", "nomePosto": "Estação de trabalho", "lotacaoMax": "0", "peso": "1", "ativo": "1" }
    ]
  }
]
```
...
`lotacaoMax` da **sala** tem um valor base, mas `ajustesCapacidade` traz **overrides por
...
`ajustesCapacidade` cobrindo a data (`dataInicio ≤ dia ≤ dataFim` e `ativo = 1`), usa
`lotacaoAjustada` dele; senão, usa `lotacaoMax` base.

`postos[].lotacaoMax = "0"` significa "capacidade não fixa no nível do posto" — a
...
`ajustesCapacidade` podem ser cadastrados no meio do expediente.
...
4. Testar sala com mais de um posto com `lotacaoMax ≠ 0` simultâneo.
