...
``/selecionar-contexto`` no ``GET /`` e precisam fixar local (``id_local`` da Connection)
via ``POST /aplicar-contexto`` antes do calendário responder. O passo é condicional — conta
...
_AUTH_MARKERS = ('name="csrf-token"', "agendaReservas")
...
_PERFIL_SCREEN_MARKER = 'id="seletorUnidade"'
# Corpo de sucesso de ``POST /aplicar-contexto`` (``text/plain`` — só o dígito ``1``).
_SET_CONTEXTO_OK = "1"
...
class Connection:
    ...
    cookie: str
    id_local: str
    # Filtro opcional de empresa na tela ``/selecionar-contexto`` (ADR-0001 D7). O BI de
...
class ReservaSpaceClient:
    ...
    def __init__(
        ...
        *,
        ...
        # HTML de ``/selecionar-contexto`` da última seleção de contexto, exposto em
        # ``fetch_raw`` para o bootstrap gravar/scrubar. ``None`` se o passo foi pulado.
        self._last_selecionar_contexto: str | None = None
    ...
    def _select_contexto(self) -> None:
        """Fixa local (e empresa, se houver) na sessão via ``POST /aplicar-contexto`` (D7).

        A tela ``/selecionar-contexto`` carrega o par CSRF nas ``<meta>`` como qualquer
        ...
        corpo ``"1"`` (``text/plain``). Cada escopo é um POST próprio, com ``filtro``
        = ``{"escopo": "local"|"empresa", "id": <id>}`` JSON.
        """
        view = self._text_or_empty_error(
            self._request("GET", "/selecionar-contexto"), "GET /selecionar-contexto"
        )
        self._last_selecionar_contexto = view
        csrf = extract_csrf(view)
        for escopo, ident in self._perfis():
            body = {"filtro": json.dumps({"escopo": escopo, "id": ident})}
            ...
            if text != _SET_CONTEXTO_OK:
                raise UpstreamError(
                    f"POST /aplicar-contexto (escopo={escopo}, id={ident}) devolveu "
    ...
    def _get_calendar_html(self) -> str:
        ...
        Sessão sem local fixado → ``GET /`` cai em ``/selecionar-contexto`` (marcador
    ...
    def _fetch_pages(self) -> tuple[str, str, dict[str, str]]:
        ...
        loc_html = self._text_or_empty_error(
            self._request(
                "GET", "/config/unidade", params={"idLocal": self._conn.id_local}
            ),
            "GET /config/unidade",
        )
        caps: dict[str, str] = {}
        for faixa_id in extract_faixa_ids(loc_html):
            body = {
                "filtro": json.dumps({"idTurno": faixa_id}),
    ...
    def fetch_raw(self) -> dict[str, str]:
        ...
        if self._last_selecionar_contexto is not None:
            raw["selecionar-contexto.html"] = self._last_selecionar_contexto
        raw.update({f"capacidade-salas_{fid}.json": text for fid, text in caps.items()})
        return raw
    ...
    def check_session(self, *, raise_on_invalid: bool = True) -> bool:
        ...
        ``/selecionar-contexto``, seleciona o contexto (D7) antes de validar.
        """
        try:
    ...
    def collect(self) -> CollectResult:
        """Executa ``GET /`` → ``GET /config/unidade`` → ``POST capacidade-salas``."""
        ...
        for faixa_id, text in caps.items():
            for sala in json.loads(text):
                sala.setdefault("idTurno", faixa_id)
