"""
Canário de integridade — contagem por status calculada × ``totaisPorStatus`` do bloco.
...
Divergência entre a contagem derivada de ``reservas[]`` e o ``totaisPorStatus``
pré-agregado do bloco sinaliza truncamento silencioso do array (mapeamento §2.1). O
``totaisPorStatus`` real vem por **bucket de workflow**, então os ``idSituacao`` são
...
# ``totaisPorStatus`` vem como mapa ``{bucket: n}`` chaveado por **bucket de workflow**,
# não por ``idSituacao``. A tolerância à lista de objetos fica como defesa.
_COUNT_KEYS = ("count", "total")
_STATUS_KEYS = ("idSituacao", "status")

# Mapa PROVISÓRIO ``idSituacao`` → bucket de ``totaisPorStatus``.
...
def bucket_do_status(id_status: Any) -> str:
    """``idSituacao`` → bucket de ``totaisPorStatus`` (mapa provisório).
...
def normalize_count_por_status(raw: Any) -> dict[str, int] | None:
    """Normaliza ``totaisPorStatus`` para ``{str(status): int}`` — ou ``None`` se ausente.

    Aceita as duas formas plausíveis da fonte: mapa ``{"reservado": 10}`` e lista de
    objetos ``[{"idSituacao": 1, "count": 10}]`` (chaves de contagem/status variam).
    """
    if raw is None:
        return None
    ...
    if isinstance(raw, list):
        ...
        for item in raw:
            if not isinstance(item, dict):
                raise SchemaError(f"item de 'totaisPorStatus' não é objeto: {item!r}")
            ...
            if status is None or count is None:
                raise SchemaError(f"item de 'totaisPorStatus' sem status/contagem: {item!r}")
            out[str(status)] = int(count)
        return out
    raise SchemaError(f"'totaisPorStatus' com forma inesperada: {type(raw).__name__}")


@dataclass(frozen=True)
class CanaryResult:
    """Resultado da checagem de um bloco. Contagens chaveadas por bucket de ``totaisPorStatus``."""

    status: str  # "match" | "mismatch" | "skipped"
    calculado: dict[str, int]  # buckets derivados de reservas[] via bucket_do_status
    esperado: dict[str, int] | None  # totaisPorStatus do bloco (já por bucket na fonte)
    detail: str = ""
...
def check_status_counts(
    ...
    *,
    ...
    """Compara a contagem por bucket derivada de ``reservas[]`` com o ``totaisPorStatus``.

    Os ``idSituacao`` são traduzidos para bucket (:func:`bucket_do_status`) antes
    da comparação, porque a fonte chaveia ``totaisPorStatus`` por bucket de workflow, não
    por id.
    """
    ...
    if esperado is None:
        return CanaryResult("skipped", calculado, None, "bloco sem 'totaisPorStatus'")
    ...
    result = CanaryResult(
        ...
        calculado,
        esperado,
        f"divergência calculado×totaisPorStatus{label}: {diffs}",
        diffs,
    )
...
def check_bloco_integrity(
    ...
    *,
    ...
    return check_status_counts(
        (rec.payload.get("idSituacao") for rec in bloco.reservas),
