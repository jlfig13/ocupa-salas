...
``hash_linha`` calculado. Blocos placeholder (``extendedProps.marcadorDia: true``) são
...
def extract_locais(html: str) -> list[Any]:
    """Array cru do global JS ``locais`` de ``/config/unidade``.
...
def extract_faixa_ids(html: str) -> list[str]:
    """Lista ordenada e sem repetição de ``idTurno`` do global JS ``locais``.

    Fonte: ``locais[].turnos[].idTurno`` (mapeamento_fonte.md §6.1).
    """
    ...
    for loc in locais:
        ...
        faixas = loc.get("turnos")
        if faixas is None:
            raise SchemaError("local sem 'turnos' no global 'locais'")
        for faixa in faixas:
            if not isinstance(faixa, dict) or "idTurno" not in faixa:
                raise SchemaError("faixa sem 'idTurno' no global 'locais'")
            fid = str(faixa["idTurno"])
            if fid not in seen:
                seen.add(fid)
                ids.append(fid)
    return ids
...
def _parse_bloco(item: Any, pos: int) -> Bloco | None:
    ...
    extendedProps = item.get("extendedProps")
    if not isinstance(extendedProps, dict):
        raise SchemaError(f"bloco {pos} sem objeto 'extendedProps'")
    if extendedProps.get("marcadorDia") is True:
        return None  # placeholder de cabeçalho de dia — ignorar (mapeamento §2.1)

    reservas_raw = extendedProps.get("reservas")
    if not isinstance(reservas_raw, list):
        raise SchemaError(f"bloco {pos} sem lista 'extendedProps.reservas'")
    ...
    return Bloco(
        ...
        count_por_status=normalize_count_por_status(extendedProps.get("totaisPorStatus")),
        reservas=registros,
    )
