...
def capture_raw(
    ...
    """Baixa ``GET /`` + ``capacidade-salas`` e grava os corpos crus em ``out_dir``.
...
def build_local_fixture_html(
    ...
    """Idem para a captura crua de ``/config/unidade``: scrub do global ``locais``."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    c = sub.add_parser("capture", help="baixa GET / + capacidade-salas crus para .captures/")
    ...
    s.add_argument(
        ...
        default="calendar",
        help="calendar = tela de calendário (events); local = /config/unidade",
    )
