...
#: Campos de ``locais[].turnos[]`` que a fixture versionada preserva — o que
...
def scrub_events(events: list[Any]) -> list[Any]:
    ...
    for item in events:
        if not isinstance(item, dict):
            ...
            continue
        block = dict(item)
        nonstd = block.get("extendedProps")
        if isinstance(nonstd, dict):
            ...
            block["extendedProps"] = new_nonstd
...
def scrub_locais(locais: list[Any]) -> list[Any]:
    ...
    Mantém ``idLocal`` e ``turnos[]`` (só a whitelist
    ...
    for loc in locais:
        ...
        faixas = [
            ...
            for f in loc.get("turnos") or []
            if isinstance(f, dict)
        ]
        out.append({"idLocal": loc.get("idLocal"), "turnos": faixas})

    assert_no_pii(out)
    return out


def scrub_capacidade_salas(salas: list[Any]) -> list[Any]:
    """Neutraliza a resposta de ``capacidade-salas`` para virar fixture versionável.
    ...
    contagens (``lotacaoMax``, ``peso``, ``postos[]``) ficam intactas.
    """
    out: list[Any] = []
    for s in salas:
