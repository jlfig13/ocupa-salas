...
Cookies aceitos em ``RSSESSID``:
  * ``demo``  → conta multi-local: ``GET /`` cai em ``/selecionar-contexto`` (ADR-0001 D7)
...
from mock_server.templates import CSRF_PARAM, calendar_page, local_page, selecionar_contexto_page
...
def create_app() -> FastAPI:
    ...
    def calendar(request: Request) -> HTMLResponse:
        ...
        if "local" not in ctx(tok):
            return HTMLResponse(selecionar_contexto_page(LOCAIS, EMPRESAS, CSRF_TOKEN))
        return HTMLResponse(calendar_page(calendar_events(), CSRF_TOKEN))

    @app.get("/selecionar-contexto", response_class=HTMLResponse)
    def escolher(request: Request) -> HTMLResponse:
        _token(request)
        return HTMLResponse(selecionar_contexto_page(LOCAIS, EMPRESAS, CSRF_TOKEN))

    @app.post("/aplicar-contexto")
    async def aplicar_contexto(request: Request):
        tok = _token(request)
        if not _require_xhr(request):
            return PlainTextResponse("", status_code=200)  # mania: corpo vazio
        form = dict(await request.form())
        _require_csrf(request, form)
        params = json.loads(form["filtro"])
        ctx(tok)[params["escopo"]] = str(params["id"])
        return PlainTextResponse("1")

    @app.get("/config/unidade", response_class=HTMLResponse)
    ...
    @app.post("/config/capacidade-salas")
    async def capacidade_salas(request: Request):
        _token(request)
        if not _require_xhr(request):
            return PlainTextResponse("", status_code=200)  # mania: corpo vazio
        form = dict(await request.form())
        _require_csrf(request, form)
        fid = str(json.loads(form["filtro"])["idTurno"])
