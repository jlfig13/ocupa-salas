...
# ids que este gerador usa. O totaisPorStatus da fonte é chaveado por bucket de workflow.
_STATUS_BUCKET = {
    1: "reservado",
    2: "cancelado",
    3: "checkIn",
...
def salas_por_faixa(id_faixa: str, *, hoje: date | None = None) -> list[dict[str, Any]]:
    """Resposta de ``POST /config/capacidade-salas`` para uma faixa (mapeamento §6.2)."""
    ...
    for id_sala, nome, base in _SALAS_BASE.get(id_faixa, []):
        ...
        if rng.random() < 0.7:
            ...
            alteracoes.append(
                {
                    "idAjuste": f"{id_sala}A1",
                    "lotacaoAjustada": str(base + rng.choice([4, 8, 12])),
        ...
        out.append(
            {
                ...
                "nome": nome,
                "idTurno": id_faixa,
                "ajustesCapacidade": alteracoes,
                "postos": [
                    {
                        "idPosto": f"{id_sala}P1",
...
def _membro(id_membro: int) -> dict[str, Any]:
    ...
    return {
        ...
        "centroCustoNomeErp": "CENTRO DE CUSTO SINTÉTICO",
        "codMembroErp": f"ERP-M-{id_membro}",
...
def _bloco(bid: int, title: str, dia: str, reservas: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "extendedProps": {"reservas": reservas, "totaisPorStatus": count},
    }
