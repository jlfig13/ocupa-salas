"""
Builders de HTML da fonte fictícia — página server-rendered com FullCalendar embutido.
...
def calendar_page(events: list[Any], csrf_token: str) -> str:
    return (
        _head(csrf_token)
        + '<div id="agendaReservas"></div><script>\n'
...
def selecionar_contexto_page(locais: list[Any], empresas: list[Any], csrf_token: str) -> str:
    ...
    return (
        ...
        + f'<select id="seletorUnidade">{opts_l}</select>'
        + f'<select id="seletorEmpresa">{opts_e}</select>'
