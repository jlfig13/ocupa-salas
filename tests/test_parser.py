...
def test_dia_normal_ignora_placeholder_e_extrai_blocos(fx):
    ...
    assert len(page.blocos) == 3  # 2 placeholders 'marcadorDia' descartados
...
def test_hash_estavel_entre_parses_e_muda_com_conteudo(fx):
    ...
    alterado = html.replace('"idSituacao": 1,', '"idSituacao": 4,', 1)
...
def test_erro_claro_bloco_sem_extended_props():
    html = 'x = {"events": [{"id": 1, "title": "08:00 - 12:00"}]}'
    with pytest.raises(SchemaError, match="extendedProps"):
        parse_page(html)


def test_erro_claro_bloco_sem_lista_reservas():
    html = 'x = {"events": [{"id": 1, "extendedProps": {"totaisPorStatus": {}}}]}'
