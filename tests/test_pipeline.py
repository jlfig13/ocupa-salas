...
def test_write_jsonl_append_preserva_linhas_anteriores(tmp_path):
    destino = tmp_path / "out.jsonl"
    write_jsonl([{"a": 1}], destino)
    write_jsonl([{"a": 2}], destino, append=True)
    write_jsonl([], destino, append=True)
    linhas = destino.read_text(encoding="utf-8").splitlines()
    assert [json.loads(linha) for linha in linhas] == [{"a": 1}, {"a": 2}]


def test_cli_coleta_repetida_nao_apaga_historico(tmp_path, capsys):
    """Duas coletas seguidas: a 2ª não traz linha nova, mas o histórico da 1ª fica no arquivo."""
    pytest.importorskip("fastapi")
    from ocupa_salas.__main__ import main

    args = ["--mock", "collect", "--out", str(tmp_path), "--particao", "2026-08-14"]
    assert main(args) == 0
    reservas = tmp_path / "reservas.jsonl"
    primeira = reservas.read_text(encoding="utf-8").splitlines()
    assert primeira

    assert main(args) == 0
    assert "novas=0" in capsys.readouterr().out
    assert reservas.read_text(encoding="utf-8").splitlines() == primeira

    capacidade = (tmp_path / "capacidade.jsonl").read_text(encoding="utf-8").splitlines()
    assert len(capacidade) == len({json.loads(c)["id_origem"] for c in capacidade})
