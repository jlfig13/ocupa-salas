...
| Arquivo | Cenário |
|---|---|
| `calendar_dia_normal.html` | 2 dias, blocos placeholder (`marcadorDia`), contagens que conferem |
| `calendar_dia_alto_volume.html` | 1 bloco, 40 reservas |
| `calendar_mes_vazio.html` | só placeholders e bloco sem reserva (um sem `totaisPorStatus`) |
| `calendar_canario_divergente.html` | `totaisPorStatus` não bate com `reservas[]` (truncamento simulado) |
| `local.html` | global JS `locais` com `turnos` + metas CSRF (Seam 2) |
| `capacidade_salas.json` | resposta de `POST /config/capacidade-salas` |
| `reserva_raw_com_pii.json` | uma reserva crua com PII sintética, para os testes de strip |
| `selecionar_contexto.html` | tela `/selecionar-contexto` (seleção de contexto, ADR-0001 D7) |
