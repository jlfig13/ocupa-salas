...
# totaisPorStatus da fonte é chaveado por bucket de workflow. Espelho do mapa de
...
_FAKE_MEMBRO = {
    ...
    "setorCodErp": "ERP-SET-1",
    "setorNomeErp": "SETOR SINTÉTICO",
    "centroCustoCodErp": "ERP-CC-9",
    "centroCustoNomeErp": "CC SINTÉTICO",
    "codMembroErp": "ERP-M-1",
    "codContratoErp": "ERP-CT-1",
    "unidadeNegocioErp": "UN SINTÉTICA",
    "id_contrato_erp": "ERP-1",
    "contrato": {
...
def _reserva(id_res: int, id_status: int, *, id_membro: int = 9000, check: bool = False) -> dict:
    ...
    return {
        ...
        "idAlocacao": 5,
        "idMembro": id_membro,
        "idSituacao": id_status,
        "idPendencia": None,
        "idPosto": "401P1",
        "idSala": "401",
        "idTurno": "1071",
        "dataReserva": "2026-08-14T08:30:00-03:00",
        "criadoPor": "USR-AAAA-0001",
        ...
        "concluidoPor": "USR-PROF-0009" if id_status == 4 else None,
        "observacao": "TEXTO LIVRE DIGITADO - risco de PII",
        "membro": membro,
        "sala": {"id": "401", "nome": "SALA SINTÉTICA"},
        "situacao": {"id": id_status, "nome": f"STATUS {id_status}"},
    }
...
def _placeholder(dia: str) -> dict:
    return {
        ...
        "start": dia,
        "end": dia,
        "extendedProps": {"marcadorDia": True, "reservas": []},
    }
...
def main() -> None:
    # Dia normal: 2 dias, placeholders, contagens que conferem com totaisPorStatus.
    normal = [
    ...
    # Alto volume: um bloco com 40 reservas; totaisPorStatus confere.
    big = [
    ...
    # Canário divergente: totaisPorStatus não bate com reservas[] (truncamento simulado).
    trunc = [
    ...
    locais = [
        {
            ...
            "turnos": [
                {"idTurno": "1071", "nomeTurno": "MANHA"},
                {"idTurno": "1072", "nomeTurno": "TARDE"},
            ],
        }
    ]
    local_html = (
    ...
    # Tela /selecionar-contexto (ADR-0001 D7): marcador id="seletorUnidade" + par CSRF.
    escolher = (
        "<!doctype html><html><head>"
        '<meta name="csrf-param" content="reservaspace_csrf">'
        '<meta name="csrf-token" content="TESTE-CSRF-TOKEN-abc123">'
        "</head><body><h1>Selecione um Local</h1>"
        '<select id="seletorUnidade"><option value="1">COWORKING CENTRAL</option></select>'
        '<select id="seletorEmpresa"><option value="10">ACME (fictícia)</option></select>'
        "</body></html>\n"
    )
    (HERE / "selecionar_contexto.html").write_text(escolher, encoding="utf-8")
    print("wrote selecionar_contexto.html")

    # Resposta de capacidade-salas (mapeamento_fonte.md §6.2).
    capacidade = [
        {
            "idSala": "401",
            "lotacaoMax": "8",
    ...
    (HERE / "capacidade_salas.json").write_text(
        json.dumps(capacidade, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    print("wrote capacidade_salas.json")
